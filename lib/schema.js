...
const frostbiteCosmetics = ['frostbite bonnet', 'frostbite fit'];
    ...
    getItemObjectFromName(name) {
        ...
        if (exception.includes(qualitySearch) === false) {
            ...
            for (const qualityC in this.qualities) {
                if (!Object.hasOwn(this.qualities, qualityC)) {
                    continue;
                }
        ...
        for (const effectC in this.effects) {
            if (!Object.hasOwn(this.effects, effectC)) {
                continue;
            }
            ...
            if (effect === 'frostbite' && frostbiteCosmetics.some(cosmetic => name.includes(cosmetic))) {
                // Skip Frostbite effect if name include Faunted Braken
                continue;
            }

            if (effect === 'sizzling' && name.includes('sizzling aroma')) {
                continue;
            }
        ...
        if (item.wear) {
            for (const paintkitC in this.paintkits) {
                if (!Object.hasOwn(this.paintkits, paintkitC)) {
                    continue;
                }
        ...
        if (name.includes('(paint: ')) {
            ...
            for (const paintC in this.paints) {
                if (!Object.hasOwn(this.paints, paintC)) {
                    continue;
                }
